...
async def matching(interaction: discord.Interaction, rival: discord.Member):
    ...
    try:
        ...
        if p1_goles > p2_goles:
            ...
            final_embed.set_footer(text="🤝 ¡Empate reglamentario! Ambos equipos comparten los puntos.")
            await message.edit(content="🎙️ **Transmisión finalizada.**", embed=final_embed)
...
async def ia_match(interaction: discord.Interaction):
    ...
    try:
        ...
        if p1_goles > p2_goles:
            ...
            final_embed.set_footer(text="🤝 ¡Empate reglamentario! La máquina y tú comparten la gloria.")
            await message.edit(content="🎙️ **Transmisión finalizada.**", embed=final_embed)
...
@app_commands.checks.has_permissions(administrator=True)
@is_in_admin_guild()
async def giveplayer(interaction: discord.Interaction, usuario: discord.Member, nombre_jugador: str):
    await interaction.response.defer(ephemeral=True)
    players_data = await get_all_players()
    matches = [p["data"] for p in players_data if nombre_jugador.lower() in p["data"]["name"].lower()]
    if not matches:
        return await interaction.followup.send("❌ Jugador no encontrado en la base de datos global.")
...
async def takeplayer(interaction: discord.Interaction, usuario: discord.Member, nombre_jugador: str):
    ...
    await save_user_profile(usuario.id, profile)
    await interaction.followup.send(f"🗑️ Se ha quitado a **{target_player['name']}** del club de {usuario.mention}.")
